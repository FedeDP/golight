...
type BacklightApi interface {
	ClightdApi
	GoSetAll(val float64, smooth *BacklightSmooth, ch chan *dbus.Call) error
	SetAll(val float64, smooth *BacklightSmooth) (bool, error)
}
...
func (api api) GoSetAll(val float64, smooth *BacklightSmooth, ch chan *dbus.Call) error {
	call := api.obj.Go(backlightMethodSetAll, 0, ch, val, smooth,"")
	return call.Err
}

func (api api) SetAll(val float64, smooth *BacklightSmooth) (bool, error) {
